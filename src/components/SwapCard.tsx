...
export default function SwapCard() {
  ...
  const handleExecuteSwap = () => {
    if (isSolanaToken(fromCoin) && isSolanaToken(toCoin)) {
      ...
      const url = `https://jup.ag/swap/${fromCoin}-${toCoin}?inputMint=${fromMint}&outputMint=${toMint}&inAmount=${fromAmount}&referralAccount=AsQ8KE5dYc4DmyCqEezGGRMu7fz4H9vmAe7dX7HAMyum&referralFee=80`;
    ...
    <div className="liquid-glass rounded-2xl p-5 w-full max-w-[440px]">
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <RefreshCw size={18} className="text-[#737373] hover:text-white cursor-pointer transition-colors" />
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        {POPULAR_PAIRS.map((pair) => (
          <button
            ...
            onClick={() => { setFromCoin(pair.from); setToCoin(pair.to); }}
            ...
            {showFromDropdown && (
              <div className="absolute right-0 top-full mt-2 bg-[#111] border border-white/[0.08] rounded-xl p-2 z-50 min-w-[160px] max-h-[200px] overflow-y-auto">
                {SUPPORTED_CRYPTOS.map((c) => (
                  <button
                    key={c.symbol}
                    onClick={() => { setFromCoin(c.symbol); setShowFromDropdown(false); }}
            ...
            {showToDropdown && (
              <div className="absolute right-0 top-full mt-2 bg-[#111] border border-white/[0.08] rounded-xl p-2 z-50 min-w-[160px] max-h-[200px] overflow-y-auto">
                {SUPPORTED_CRYPTOS.map((c) => (
                  <button
                    key={c.symbol}
                    onClick={() => { setToCoin(c.symbol); setShowToDropdown(false); }}
